...
function start_contact_generation(df::DataFrame, parameters::Contact_simulation_options)
	...
	for school in 1:parameters.school_place_num
		...
		for couple in combinations_iterator
				...
				add_event(event_writer,Event(student_1.id,student_1.work_school_place,PLACE_SCHOOL,DateTime(year,month,day,contact_hour)))
				add_event(event_writer,Event(student_2.id,student_2.work_school_place,PLACE_SCHOOL,DateTime(year,month,day,contact_hour)))
				cont += 2
		end
		write_events(event_writer)
	end
